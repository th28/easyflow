...
class GraphCanvas(tk.Canvas):
    ...
    def __init__(self, window, width, height, background, graph_model):
        ...
        self.entry = None
        self.objective_text = self.create_text(300, 20, text="Objective value: ")
        self.button_solve = tk.Button(self, text="Solve", command=self.solve, anchor=tk.W)
        self.button_reset = tk.Button(self, text="Reset", command=self.reset_graph, anchor=tk.W)
        self.button_reset_window = self.create_window(10, 40, anchor=tk.NW, window=self.button_reset)
        self.button_solve_window = self.create_window(10, 10, anchor=tk.NW, window=self.button_solve)
        self.node_size = 15
        self.guide = None

    def solve(self):
        sol_flow, obj_val = solver(self.graph_model)
        max_flow = max(list(sol_flow.values()))
        min_flow = min(list(sol_flow.values()))

        self.itemconfig(self.objective_text, text="Objective value: " + str(obj_val))

        for edge in sol_flow.keys():
            edge_id = self.id_to_edge[edge]
            text_id = self.item_id_to_text[edge_id]
            flow = sol_flow[edge]
            self.itemconfig(text_id, text=str(flow))
            try:
                self.itemconfig(edge_id, width=10*(flow - min_flow) / (max_flow - min_flow))
            except ZeroDivisionError:
                self.itemconfig(edge_id, width=1)

    def reset_graph(self):
        self.graph_model.nodes = {}
        self.graph_model.edges = {}
        self.node_count = 0
        self.delete("node")
        self.delete("text")
        self.delete("edge")
    ...
    def draw_line(self, event):
        ...
        if self.is_hovering_item == "node":
            if self.click_number == 0:
                ...
                self.id_to_edge[(self.start_node, self.end_node)] = edge_id
